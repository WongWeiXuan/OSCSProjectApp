...
public class Client {
	...
	private static void sendPacket() {
		try {
			...
			byte[] sendData = "YOUALIVE?".getBytes();
			...
			// We have a response
			String packetData = new String(receivePacket.getData()).trim();
			...
			if ("YEA,I'MALIVE.HOWABOUTYOU?".equals(packetData)) {
				...
				responseBuffer = "IMFINETHANKYOU".getBytes();
				DatagramPacket sendPacket = new DatagramPacket(responseBuffer, responseBuffer.length,
						receivePacket.getAddress(), receivePacket.getPort());
				broadcastSocket.send(sendPacket);

				byte[] responseBuffer2 = new byte[10000];
				responseBuffer2 = "FILETRANSFER".getBytes();
