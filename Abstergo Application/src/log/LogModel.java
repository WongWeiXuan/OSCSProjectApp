...
public class LogModel {
	...
	public int getNewestID(File encrypted, String key) {
		try {
			AESEncryption aes = new AESEncryption(Base64.getDecoder().decode(key.getBytes()));
	...
	public File read(String aesKey) {
	    ...
	    while(true) {
	    	if("System".equals(logName)) {
	    		if (!Advapi32.INSTANCE.ReadEventLog(handle, WinNT.EVENTLOG_SEQUENTIAL_READ | WinNT.EVENTLOG_BACKWARDS_READ, 0, buffer, (int) buffer.size(), pnBytesRead, pnMinNumberOfBytesNeeded) && !stop) {
	...
	private String getString(EVENTLOGRECORD record, Pointer pevlr) {
		...
		if (record.DataLength.intValue() > 0) {
			byte[] data = pevlr.getByteArray(record.DataOffset.intValue(),
					record.DataLength.intValue());
			String[] stringArray = new String[1];
			stringArray[0] = new String(data);
			ss = stringArray;
		}
		// Strings
